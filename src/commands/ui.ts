    ...
    app.post('/api/search', async (c) => {
      try {
        ...
        try {
          ...
          const result = await client.execute({ sql, args });
          const results = result.rows.map((row) => parseSearchRow(row as Record<string, unknown>));
          // Track usage for returned results
          await trackUsage(results.map(r => r.id));
          return c.json({ success: true, data: { query, results } });
        } catch {
          // Fallback to non-indexed search
          const fallbackConditions: string[] = ['active = 1'];
          const fallbackArgs: InValue[] = [JSON.stringify(queryEmbedding)];
