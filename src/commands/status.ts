...
export const statusCommand = new Command('status').description('Check Turso connection status').action(async () => {
  try {
    // Check config
    if (!configExists()) {
      const response: CliResponse = {
        success: false,
        error:
          'Not configured. Create .superintent/.env with TURSO_URL (and TURSO_AUTH_TOKEN for cloud), then run: superintent init',
      };
