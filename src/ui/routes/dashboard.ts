...
import { renderDashboardGrid, renderDashboardView } from '../components/index.js';
  ...
  app.get('/partials/dashboard-grid', async (c) => {
    try {
      ...
      try {
        ...
        try {
          const ticketTotalResult = await client.execute(`SELECT COUNT(*) as total FROM tickets`);
