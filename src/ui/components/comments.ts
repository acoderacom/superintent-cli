...
function renderCommentCard(comment: Comment): string {
  ...
  return `
    <div class="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3 group" id="comment-${escapeHtml(comment.id)}">
      <div class="flex items-center justify-between mb-1">
        <div class="flex items-center gap-2">
          ${authorBadge}
          <span class="text-xs text-gray-400 dark:text-gray-500">${comment.created_at ? new Date(comment.created_at).toLocaleString() : ''}</span>
        </div>
        <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button type="button"
                  class="p-1 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 rounded transition-colors cursor-pointer"
                  title="Edit comment"
          ...
          <button type="button"
                  class="p-1 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 rounded transition-colors cursor-pointer"
      ...
      <div class="text-sm text-gray-700 dark:text-gray-200">${escapeHtml(comment.text)}</div>
    </div>
  `;
}
...
export function renderEditCommentForm(comment: Comment): string {
  return `
    <div class="bg-gray-100 dark:bg-gray-700/50 rounded-lg p-3" id="comment-${escapeHtml(comment.id)}">
